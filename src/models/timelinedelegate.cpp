                             ...
                             const QModelIndex& index) const
{
    ...
    const auto threadTimeRect = QRect(QPoint(data.mapTimeToX(data.threadStartTime),
                                      ...
                                      data.h));
    painter->fillRect(threadTimeRect, runningColor);
                                 ...
                                 const QModelIndex& index)
{
    if (event->type() == QEvent::ToolTip) {
        ...
        const auto formattedTime = Util::formatTime(time - data.minTime);
        if (totalCost > 0) {
            QToolTip::showText(event->globalPos(), tr("time: %1\nsamples: %2\ntotal sample cost: %3\nmax sample cost: %4")
                .arg(formattedTime).arg(std::distance(firstIt, lastIt))
